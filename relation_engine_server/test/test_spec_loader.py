...
class TestSpecLoader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_spec_dir = os_path.join(
            cls.test_dir, "spec_release", "sample_spec_release", "spec"
        )
        ...
        for key in cls.config["spec_paths"].keys():
            if cls.repo_path in cls.config["spec_paths"][key]:
                cls.config["spec_paths"][key] = cls.config["spec_paths"][key].replace(
                    cls.repo_path, cls.test_spec_dir
                )
    ...
    def tearDownClass(cls):
        ...
        for key in cls.config["spec_paths"].keys():
            if cls.test_spec_dir in cls.config["spec_paths"][key]:
                cls.config["spec_paths"][key] = cls.config["spec_paths"][key].replace(
                    cls.test_spec_dir, cls.repo_path
                )
    ...
    def test_run_spec_loading_tests(self, schema_type_names=[], test_name=None):
        ...
        for schema_type in schema_type_names:
            self.assertEqual(
                result_path, spec_loader.get_schema(schema_type, test_name, True)
            )
        ...
        if schema_type_singular == "data_source":
            ...
            self.assertIn(
                endpoint.scheme + "://" + endpoint.netloc, result_obj["logo_url"]
            )
        ...
        err_msg = (
            schema_type_singular.capitalize().replace("_", " ")
            + " '"
            + fake_name
            + "' does not exist."
        )
    ...
    def test_get_schemas_of_various_types(self):
        ...
        for schema in schema_type_list:
            with self.subTest(schema=schema["schema_type_names"][0]):
                self.test_run_spec_loading_tests(
                    schema["schema_type_names"], schema["example"]
                )
                if "names" in schema:
    ...
    def test_prevent_non_spec_dir_access(self):
        ...
        result = spec_loader.get_schema(
            "stored_queries", path_in_spec_repo, path_only=True
        )
        self.assertEqual(
            result,
            os_path.join(
                self.test_spec_dir, "stored_queries", "test", "fetch_test_vertex.yaml"
            ),
        )
        ...
        with self.assertRaisesRegex(SchemaNonexistent, err_msg):
            spec_loader.get_schema(
                "stored_queries", path_outside_spec_repo, path_only=True
            )
