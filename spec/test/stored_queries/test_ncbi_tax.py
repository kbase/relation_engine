...
class TestNcbiTax(unittest.TestCase):
    ...
    def test_get_lineage_valid(self):
        ...
        resp = requests.post(
            ...
            data=json.dumps(
                {"ts": _NOW, "id": "7", "select": ["rank", "scientific_name"]}
            ),
        ).json()
    ...
    def test_siblings_valid(self):
        ...
        resp = requests.post(
            ...
            data=json.dumps(
                {"ts": _NOW, "id": "5", "select": ["rank", "scientific_name"]}
            ),
        ).json()
        result = resp["results"][0]
        self.assertEqual(result["total_count"], 2)
    ...
    def test_search_sciname_more_complicated(self):
        ...
        resp = requests.post(
            ...
            params={"stored_query": "ncbi_taxon_search_sci_name"},
            data=json.dumps(
                {"ts": _NOW, "search_text": "prefix:gamma,|prefix:alpha,|prefix:delta"}
            ),
        ).json()
        result = resp["results"][0]
    ...
    def test_search_sciname_offset_max(self):
        ...
        resp = requests.post(
            ...
            params={"stored_query": "ncbi_taxon_search_sci_name"},
            data=json.dumps(
                {"ts": _NOW, "search_text": "prefix:bact", "offset": 100001}
            ),
        )
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(
...
def _run_search_sciname(
    ...
    """
    data = {"ts": _NOW, "search_text": "prefix:bac"}
